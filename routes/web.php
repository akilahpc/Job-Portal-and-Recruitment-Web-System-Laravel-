...
Route::group(['prefix' => 'account'],function(){  //mehi ['account'] kiynne sama route ekkma /account use karala tiyana nisa eka array ekk ('/account/register')

  //Guest Route  
  
  //(kernel.php) eken path ek balagana ->  (*'guest'* => \App\Http\*Middleware*\RedirectIfAuthenticated) -> ekt yanna

  Route::group(['middleware'=>'guest'], function(){
    Route::get('/account/register',[AccountController::class,'registration'])->name('account.registration');
    Route::post('/account/process-register',[AccountController::class,'processRegistration'])->name('account.processRegistration');
    Route::get('/account/login',[AccountController::class,'login'])->name('account.login');
    Route::post('/account/authentiate',[AccountController::class,'authentiate'])->name('account.authentiate');
  });

  //Authenticated routes

  //(kernel.php) eken path ek balagana ->  (*'auth'* => \App\Http\*Middleware*\Authenticate) -> ekt yanna
