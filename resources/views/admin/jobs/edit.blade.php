  ...
  <div class="container py-5">
      ...
      <div class="row">
          <div class="col-lg-3">
            ...
            @include('admin.sidebar')  <!-- link the sidebar -->

          </div>
          <div class="col-lg-9">

            @include('front.message')   <!-- profile updated successfully msg -->
             ...
             <div class="card-body card-form p-4">
                    ...
                    <div class="col-md-6  mb-4">
                        ...
                        <select name="category" id="category" class="form-control">
                            ...
                            @if($categories->isNotEmpty())
                                ...
                                <option {{ ($job->category_id == $category->id) ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->name }}</option> <!--mehi { ($job->category_id == $category->id) ? 'selected' : '' }}  walin drop down list eken danata select karla tiyana ek form eke penwai-->
                                @endforeach
                            @endif
                ...
                <div class="row">
                    ...
                    <div class="col-md-6  mb-4">
                        ...
                        <input value="{{ $job->vacancy }}" type="number" min="1" placeholder="Vacancy" id="vacancy" name="vacancy" class="form-control">  <!--mehi 'value="" ' walin ek ek feeld ekata adalawa danata db eke save wela tiyana data form eke display karai-->
                        <p></p>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    </div> <!--mehi 'value="" ' walin ek ek feeld ekata adalawa danata db eke save wela tiyana data form eke display karai-->

                    <div class="mb-4 col-md-6">
                        <label for="" class="mb-2">Location<span class="req">*</span></label>
                        <input value="{{ $job->location }}" type="text" placeholder="location" id="Location" name="location" class="form-control">
                        <p></p> <!--mehi 'value="" ' walin ek ek feeld ekata adalawa danata db eke save wela tiyana data form eke display karai-->
                    </div>
                </div>
                ...
                <div class="row">
                  <div class="mb-4 col-md-6">
                    <div class="form-check">
                      <input {{ ($job->isFeatured == 1) ? 'checked' : '' }} class="form-check-input" type="checkbox" value="1" id="isFeatured" name="isFeatured"> <!-- { ($job->isFeatured == 1) ? 'checked' : '' }->'jobs'table eke 'isFeatured' feld eke athe 1 da -->
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control" name="description" id="description" cols="5" rows="5" placeholder="Description">{{ $job->description }}</textarea>   <!--mehi '{ $job->description }' walin ek ek feeld ekata adalawa danata db eke save wela tiyana data form eke display karai-->
                    <p></p>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                  ...
                  <select name="experience" id="experience" class="form-control">
                      <option value="1" {{ ($job->experience == 1) ? 'selected' : '' }}>1 Year</option><!-- { ($job->experience == 1) ? 'selected' : ''}} meken kiyanne 1 Year select karala tiyanawa nam eka form eke display krnna-->
...
<script type="text/javascript"> 
    $("#editJobForm").submit(function(e){
        ...
        $.ajax({
            url:'{{ route("admin.jobs.update",$job->id) }}',   //route name
            type:'PUT',
            dataType:'json',
            data: $('#editJobForm').serializeArray(),
            success:function(response){
              $("button[type='submit']").prop('disabled',false);
                if(response.status==true){    //{173-206}  form eke aniwaren fill kala yuthu '*' ek ek feelds fill krla nam error msg ek ain kirima
                    ...
                    window.location.href="{{ route('admin.jobs') }}";  //'account.myJobs' -> route name

                }else{
                    var errors = response.errors;

                  if(errors.title){    //{188-269}  form eke aniwaren fill kala yuthu '*' ek ek feeld fill krla naththan error msg ekk penwima
    ...
    function deleteUser(id){  // delete button onclick function
        if(confirm("Are you sure you want to delete?")){
            $.ajax({
                url:'{{ route("admin.users.destroy") }}',
                type: 'delete',   //'delete' -> route method ek
                data:{id: id},
                dataType:'json',
                success:function(response){
